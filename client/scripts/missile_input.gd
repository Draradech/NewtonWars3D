...
func update_label(pl: Player, digit):
	...
	pitchstring = pitchstring.insert((4 if digit < 0 else 3) - digit, "[color=%s]" % Global.config["color_self"].to_html(false))
	...
	yawstring = yawstring.insert((4 if digit < 0 else 3) - digit, "[color=%s]" % Global.config["color_self"].to_html(false))
	...
	speedstring = speedstring.insert((4 if digit < 0 else 3) - digit, "[color=%s]" % Global.config["color_self"].to_html(false))
